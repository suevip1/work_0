...
public class TaskResourceBeanConfig {
    ...
    static {
        ...
        map1.put("spark.driver.cores","Driver程序使用的CPU核数,默认为1");
        map1.put("spark.driver.memory","Driver程序使用内存大小,默认1g");
        map1.put("spark.executor.instances","启动的executor的数量，默认为1");
        map1.put("spark.executor.cores","每个executor使用的CPU核数，默认为1");
        map1.put("spark.executor.memory","每个executor内存大小,默认1g");
        ...
        map3.put("dtscript.worker.memory","每个worker所占内存，比如512m");
        map3.put("dtscript.worker.cores","每个worker所占的cpu核的数量");
        map3.put("dtscript.worker.num","worker数量");
