...
public class JobGraphUtils {
    ...
    public static Date getPreJob(Date currTriggerDate, ScheduleCron cron) {
        ...
        if (ESchedulePeriodType.CALENDER.getVal() == cron.getPeriodType()) {
            ...
            String nearestTime = calenderParser.getNearestTime(dateTime,false);
            if(StringUtils.isBlank(nearestTime)){
                logger.error("can't find preJob:{} in calender:{}!", currTriggerDate, cron);
                return null;
            }
            return DateUtil.parseDate(nearestTime,DateUtil.UN_STANDARD_DATETIME_FORMAT);
        }
