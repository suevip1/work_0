...
public class XmlFileUtil {
    ...
    public static List<File> filterXml(List<File> xmlFiles, List<String> validXml) {
        if (xmlFiles == null) {
            throw new RdosDefineException(ErrorCode.FILE_MISS.getDescription()+ ":" + StringUtils.join(BASE_XML, ","));
        }
        if(null == validXml){
        ...
        if (!checkFiles.isEmpty() || !validXml.isEmpty()) {
            throw new RdosDefineException(ErrorCode.FILE_MISS + ":" + StringUtils.join(checkFiles, ",")+StringUtils.join(validXml, ","));
        }
        return xmlFiles;
    }
