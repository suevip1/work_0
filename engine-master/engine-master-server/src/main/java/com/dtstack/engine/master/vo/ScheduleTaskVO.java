    ...
    private void init() {
        ...
        if (StringUtils.isNotBlank(this.getScheduleConf())) {
            try {
                ...
                if (ETaskGroupEnum.MANUAL.getType().equals(getTaskGroup())) {
                    scheduleConf = ScheduleConfUtils.buildManualTaskScheduleConf(getScheduleConf());
                }
                ScheduleCron cron = ScheduleFactory.parseFromJson(scheduleConf, null, null, null);
                ...
                if (ESchedulePeriodType.MIN.getVal() == cron.getPeriodType()) {
                    ...
                    taskPeriodType = "月任务";
                } else if (ESchedulePeriodType.CUSTOM.getVal() == cron.getPeriodType()) {
                    taskPeriodType = "自定义任务";
                } else if (ESchedulePeriodType.CALENDER.getVal() == getPeriodType()) {
                    taskPeriodType = "日历任务";
                }
            } catch (Throwable e) {
