...
public class MysqlLogStore extends AbstractLogStore {
    ...
    public void updateModifyTime(Collection<String> jobIds) {
        ...
        try {
            ...
            List<List<String>> partitionJobIds = Lists.partition(new ArrayList<>(jobIds), 20);
            for (List<String> partitionJobId : partitionJobIds) {
                for (String jobId : partitionJobId) {
                    pstmt.setString(1, jobId);
                    pstmt.addBatch();
                }
                pstmt.executeBatch();
    ...
    public Integer getStatusByJobId(String jobId) {
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                Long timeout = Long.parseLong(dataConnPool.getConfigValue(RDB_TIME_OUT, String.valueOf(DEFAULT_TIMEOUT)));
                if (gmtModified.getTime() < System.currentTimeMillis() - timeout){
                    LOGGER.warn("getStatusByJobId {} timeout",jobId);
            ...
            LOGGER.error("getStatusByJobId {} error",jobId, e);
        } finally {
