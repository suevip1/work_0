...
public class SftpFileManage implements IFileManage {
    ...
    private String getSftpPoolKey(SftpConfig sftpConfig) {
        return sftpConfig.getHost() + sftpConfig.getPort() + sftpConfig.getUsername() + sftpConfig.getPassword();
    }
    ...
    public String cacheOverloadFile(String fileName, String remoteDir, String localDir) {
        ...
        try {
            boolean successDownload = downloadFile(remoteFile, localFile);
            if (!successDownload) {
                throw new RuntimeException(String.format("获取%s文件%s失败", remoteDir, fileName));
            }
    ...
    public boolean isFileExist(String sftpPath) {
        ...
        try {
            channelSftp = getChannelSftp();
            return this.isFileExist(channelSftp, sftpPath);
        } catch (Exception e) {
            LOGGER.error("isFileExist error:{}", sftpPath, e);
            throw new RdosDefineException(e);
        } finally {
            close(channelSftp);
        }
