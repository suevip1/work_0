...
public class DateUtil {
    ...
    public static final String STANDARD_FORMAT = "HH:mm";
    ...
    public static ThreadLocal<Map<String,SimpleDateFormat>> datetimeFormatter = ThreadLocal.withInitial(() -> {
        ...
        SimpleDateFormat standardDateFormatter = new SimpleDateFormat(STANDARD_DATE_FORMAT);
        standardDateFormatter.setTimeZone(timeZone);
        formatterMap.put(STANDARD_DATE_FORMAT,standardDateFormatter);
    ...
    */
    ...
    public static Date parseDate(String dateStr, String pattern,boolean lenient) throws Exception{
        SimpleDateFormat sdf = new SimpleDateFormat();
        sdf.applyPattern(pattern);
    ...
    public static final Date strictParseDate(final String dateStr, final String pattern) {
        try {
            return parseDate(dateStr, pattern, false);
        } catch (Exception e) {
    ...
    public static Long getExecTimeSecond(Timestamp execStartTime, Timestamp execEndTime, Long execTime, Integer status) {
        if (execTime != null && execTime != 0) {
            return execTime;
        }

        if (execStartTime == null && execEndTime == null) {
            return null;
        }

        if (execStartTime != null && execEndTime != null) {
            return (execEndTime.getTime() - execStartTime.getTime()) / 1000;
        }

        if (execStartTime != null) {
            // 如果任务状态已经是停止状态了，但是没有失败时间，则不返回运行时长
            if (RdosTaskStatus.isStopped(status)) {
                return null;
            }
            return (System.currentTimeMillis() - execStartTime.getTime()) / 1000;
        }

        return null;
    }

    public static String getExecTime(Timestamp execStartTime, Timestamp execEndTime, Long execTime, Integer status) {
        if (execTime != null && execTime != 0) {
        ...
        if (execStartTime != null && execEndTime != null) {
            return DateUtil.getTimeDifference(execEndTime.getTime() - execStartTime.getTime());
        }
    ...
    public static String formatTimeToScheduleTrigger(String timeStr) {
        return addTimeSplit(getTimeStrWithoutSymbol(timeStr));
    }
